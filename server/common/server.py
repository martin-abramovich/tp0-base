...
class Server:
    ...
    def run(self):
        while self._running:
            try:
                ...
                self.__handle_client_loop(client_sock)
            except OSError:
                break
    ...
    def __handle_client_loop(self, client_sock):
        """Maneja múltiples batches del mismo cliente hasta FIN_APUESTAS."""
        try:
            while True:
            ...
            logging.error(f"action: client_error | result: fail | error: {e}")
        finally:
            try:
                client_sock.close()
